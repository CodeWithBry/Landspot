...
export async function sendMail(req: Request, res: Response) {
    ...
    try {
        console.log(html, subject, agent_name, agent_email, user_id, sender_email, sender_id, sender_name)
...
export const getMailById = async (req: Request, res: Response) => {
    try {
        ...
        const query = `SELECT * FROM mails WHERE mail_id = $1`;
        const result = await pool.query(query, [mail_id]);
...
export const getUnseenMailsLength = async (req: Request, res: Response) => {
    try {
        const query = `SELECT * FROM mails WHERE mail_id = $1 AND is_seen = $2`;
