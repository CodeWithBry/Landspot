  ...
  return (
    <div className="md:flex-row flex flex-col gap-2 w-full px-5 py-3 text-serif border-2 rounded-xl border-primary-300 hover:border-primary-600">
      <div className="md:w-30 md:h-30 w-full h-70 shrink-0 rounded-2xl flex justify-center place-items-center">
        <div
          className={`md:w-full md:h-full w-full h-70`}
          style={{ ...imageStyle }} />
      </div>
      <div className="md:flex-row flex flex-col w-full">
        ...
        <div className="flex gap-2 md:w-fit w-full">
          <Link 
            href={`/dashboard/listings/${listing.id}`} className="md:w-fit w-full h-fit border-2 border-accent-400 text-accent-400 hover:bg-accent-400 hover:text-white transition text-md font-serif py-3 px-5 rounded-md cursor-pointer active:bg-accent-600">Edit</Link>
          <button
            onClick={() => handleDelete(listing.id)} 
            className="md:w-fit w-full h-fit border-2 border-danger-500 text-danger-500 hover:bg-danger-500 hover:text-white transition text-md font-serif py-3 px-5 rounded-md cursor-pointer active:bg-accent-600">Delete</button>
        </div>
      </div>
    </div>
  )
